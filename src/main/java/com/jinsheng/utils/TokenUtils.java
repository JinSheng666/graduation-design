...
public class TokenUtils {
    ...
    @Value("${graduation_design.expire-time}")
    ...
    //token中存放用户名对应的名字
    private static final String CLAIM_NAME_CODE = "CLAIM_NAME_USERCODE";
    //token中存放用户真实姓名对应的名字
    ...
    public String loginSign(CurrentUser currentUser, String password){
        ...
        stringRedisTemplate.opsForValue().set(token, token, expireTime, TimeUnit.MILLISECONDS);
    ...
    public CurrentUser getCurrentUser(String token) {
        ...
        String Code = decodedJWT.getClaim(CLAIM_NAME_CODE).asString();//用户账号
        ...
        return new CurrentUser(userId,Code,userName);
